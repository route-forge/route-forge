/**
 * useForgeByPrefix：带名字前缀的封装，方便后续减少名称传入
...
export function useForgeByPrefix(prefix: string, separator = '.'): UseForgeByPrefixReturn {
  const forge = useForge();

  const join = (suffix: string) => (suffix ? `${prefix}${separator}${suffix}` : prefix);

  return {
    api: (suffix, params) => forge.api(join(suffix), params),
    route: (suffix, params) => forge.route(join(suffix), params),
  };
}
