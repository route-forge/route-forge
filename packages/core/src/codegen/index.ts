 ...
 * 生成 d.ts 内容
 */
export function generateRouteTypes(routes: Record<string, RouteMeta>): string {
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<void> {
  ...
  const merged: Record<string, RouteMeta> = {};
  let failedLevels = 0;
  levelFetches.forEach((res, idx) => {
    if (res.status === 'fulfilled') {
      Object.assign(merged, res.value.routes);
    } else {
      failedLevels++;
  ...
  const dts = generateRouteTypes(merged);
  ...
  console.log(`[route-forge/codegen] written ${Object.keys(merged).length} routes to ${outPath}`);
  if (failedLevels > 0) {
