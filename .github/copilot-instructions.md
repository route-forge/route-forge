...
- `packages/vue` — `@route-forge/vue`: Vue 3 plugin + composables (`useForge` / `useForgeApi` / `useForgeRoute`)
- `packages/react` — `@route-forge/react`: React Provider + hooks (same names)
...
Test baseline: 276 cases (core 224 / vue 25 / react 27) — all must pass before committing.
...
- `useForgeRoute` degrades render-time errors to `''` with a styled `console.warn`; rendering must never throw.
