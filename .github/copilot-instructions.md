...
Test baseline: 333 cases (core 250 / vue 39 / react 44) — all must pass before committing.
...
- Declarative interceptors (`createRouteForge({ interceptors })`) describe exactly **one** interceptor per key, in one of three shapes: a function (→ resolve), a `[resolve?, reject?]` tuple, or a `{ resolve?, reject? }` object (normalized by `normalizeInterceptorDeclaration`; invalid shapes throw `TypeError`). Never reintroduce the old "top-level array = interceptor list" semantics; multiple interceptors go through runtime `forge.interceptors.request/response.use()`.
