...
2. **初始化时序**：`createRouteForge` 返回即后台启动 auto-discovery（摘要端点）→ 折算 `DiscoveryState` → eager 预加载 → `ready()` settle。`route()` / `hasRoute()` 有 `RF_FE_010` 守卫；`api()` 内部 await discovery，不受守卫影响。失败语义：显式 `levels` 可降级（warn），否则 `ready()` reject——不要吞掉 reject。
